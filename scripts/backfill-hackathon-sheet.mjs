/**
 * One-time backfill: push all Hackathon registrations from Supabase into the Google Sheet.
 * Skips rows whose ID is already in column A. Does not overwrite existing sheet rows.
 *
 * Usage: npm run backfill:hackathon-sheet
 */
...
async function main() {
  ...
  if (!sheetId || !rawJson) {
    throw new Error("Missing GOOGLE_HACKATHON_SHEET_ID or GOOGLE_SERVICE_ACCOUNT_JSON in .env.local");
  }
  ...
  console.log("Fetching Hackathon registrations from Supabase...");
  const { data: registrations, error } = await supabase
    .from("registrations")
    .select("*")
    .eq("main_category", "Hackathon")
    .order("created_at", { ascending: true });

  if (error) throw new Error(`Supabase query failed: ${error.message}`);
  console.log(`Found ${registrations.length} Hackathon registration(s) in database.`);
