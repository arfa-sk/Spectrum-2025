...
// Extended WebGLProgram type with custom uniform locations
...
void main(){gl_Position=position;}`;
    ...
    init() {
      ...
      const extendedProgram = program as ExtendedWebGLProgram;
    ...
    render(now = 0) {
      ...
      const extendedProgram = program as ExtendedWebGLProgram;
      if (extendedProgram.resolution) gl.uniform2f(extendedProgram.resolution, this.canvas.width, this.canvas.height);
      if (extendedProgram.time) gl.uniform1f(extendedProgram.time, now * 1e-3);
      if (extendedProgram.move) gl.uniform2f(extendedProgram.move, this.mouseMove[0], this.mouseMove[1]);
      if (extendedProgram.touch) gl.uniform2f(extendedProgram.touch, this.mouseCoords[0], this.mouseCoords[1]);
      if (extendedProgram.pointerCount) gl.uniform1i(extendedProgram.pointerCount, this.nbrOfPointers);
      if (extendedProgram.pointers) gl.uniform2fv(extendedProgram.pointers, this.pointerCoords);
  ...
  useEffect(() => {
    ...
    // Classes are defined inside the hook and are stable, no need to include in deps
